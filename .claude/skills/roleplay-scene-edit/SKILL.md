...
## 0. マージスクリプトはテンプレートから作る（ゼロから書かない）

`scripts/merge_template.js`（このスキル同梱）をscratchpadへコピーし、**`TRANSFORM`関数だけを書き換えて**使う。抽出・1:1照合の例・フィールド順序の保存・書き戻し・件数検証はテンプレート側が保証済み。`DRY_RUN=1`で書き込みなしの検証ができる。**新フィールドを追加する時はテンプレートの`FIELD_ORDER`にも追加**（実物との順序不一致は実行時に自動検出される）。

翻訳・分類など曖昧な判断を伴う大量データの生成は、コード修正と分離して**自己完結プロンプトのサブエージェント**に投げる（`{cat, title, ...}`のJSONを渡し、`cat`+`title`をバイト一致で返させ、返ってきたデータをテンプレートで1:1照合マージする。lineJa/keysJa/hintWordで実績のあるパターン）。
