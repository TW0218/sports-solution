...
### ROLEPLAY_SCENESの一括加工時は「3視点セルフレビュー」を必須とする

175エントリへの一括フィールド追加・並び替え・削除など、`ROLEPLAY_SCENES`全体に及ぶ機械的加工を行った後は、**必ず以下3つの異なる視点で加工結果をレビューしてから commit/push する**（hintフィールド全シーンズレ事故の再発防止策）。

1. **データ整合性視点**: 加工前後で「シーン総数・カテゴリ別内訳・全title集合」が完全一致するか（`git diff`やNode.jsスクリプトで機械的に検証。1件でもtitleが増減・重複していないか）
2. **意味・文脈視点**: 加工結果（並び順・新フィールドの値など）を数シーン抜き出し、その`situation`/`line`/`keys`と突き合わせて意味的に整合しているかスポットチェック（例: 「Day 1」に割り当てたシーンが本当に加入初日に起こりうる内容か）
3. **音声整合性視点**: `node scripts/validate_audio.js`が0件エラーで通ること。特に`audio`/`keyAudio`/`keys`フィールドが加工前後で**一切変化していない**ことを`git diff`で確認する（加工対象外のフィールドに意図せず触れていないかの最終チェック）

3つとも問題なければ commit/push してよい。1つでも引っかかれば加工をやり直す。

**規模に応じて自己レビューと独立エージェントレビューを使い分ける**:
- **〜50シーン程度の並び替え・限定的な変更**: 上記3視点の自己レビューで十分（コストと安全性のバランスが取れている）
- **175シーン全体に及ぶ一括加工**（新フィールドの全件追加、大規模な構造変更など）: 自己レビューに加えて、この会話の文脈を持たない独立エージェント（Explore/general-purposeエージェントなど）に上記3視点のうち最低1つ（特にデータ整合性視点）を独立検証させる。自分の思い込みによる見落としは自己レビューだけでは防げないため
...
### ROLEPLAY キャリアモード
- 7カテゴリ（pitch/manager/medical/locker/media/agent/daily）×25シーンを、カテゴリ別ではなく **「Day 1〜25」の横断ローテーション**で提示する
- 1つのDayは各カテゴリの配列内index Nのシーンを1つずつ集めた7シーン構成（`rpDayScenes(dayIdx)`）。カテゴリの内部並び順がそのままキャリアの時系列（新人期→中堅→ベテラン）を表すため、**media/agentカテゴリは加入初期〜終盤の順に手動で並び替え済み**（他5カテゴリは元の並びのままで違和感がないため未変更）
- Day Nは前のDay（N-1）を完了しないと解放されない（`rpDayUnlocked()`）。完了済みDayは何度でも再訪問・再挑戦できる（一度きりで消えない）
- 進捗は新しいSフィールドを持たず、既存の`S.rpCompletedSceneKeys`から`rpDayCompleted()`で導出する（完了判定はそのDayの7シーン全てのキーが含まれているか）
- 旧・カテゴリ自由選択グリッド（`rp-cat-grid`等）は廃止済み。カテゴリ軸の練習に戻したい場合は`RP_CATEGORIES`をベースに作り直すことになる
