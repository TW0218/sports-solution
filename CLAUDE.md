...
- **MASTERY RANK（Day25で終わらせない設計）**: 新規シーン・新規音声を追加せず、既存175シーンを難易度違いで繰り返し使わせる仕組み。`S.rpMastery`（`{sceneKey: 0〜4}`、単語カードの`S.box`パターンを流用）でシーンごとの習熟度を管理。Tier1=BRONZE（3択・既存フロー）→Tier2=SILVER（自由回答＋ヒント）→Tier3=GOLD（自由回答・ヒントなし）→Tier4=TOP BINS（同+スピードボーナス）。`rpNextTier(scene)`が次に挑戦すべきTierを返し、`renderRPScene()`がTier1なら`renderRPChoose()`、Tier2以上なら`renderRPFreeResponse()`（自由回答→MATCHDAYと同じ`mdAiScore()`で採点）に分岐する。**合格点(50)以上かつ現在の習熟度を上回るTierでのみ昇格。失敗しても習熟度は下がらない（優しめルール、単語カードの「間違えたら0に戻る」とは意図的に変えている）**。Dayグリッドには完了Dayの最低Tierをバッジ表示（`rpDayMasteryTier`）。既存の`rpDayCompleted`/`rpDayUnlocked`（Day解放条件）はTier1基準のまま変更していない。
  ...
  - **NPCセリフの日本語訳を常時表示（`lineJa`、2026-07-15）**: 中学2年生レベルの学習者ペルソナを含む10人視点分析で、初級者はNPCの英語命令文（175件中46%が疑問文ではなく命令・宣言文）自体を読めず、そもそも何を返せばいいか分からないまま離脱するリスクが最有力と判定。既存の`hint`（返答方針の日本語ヒント）とは別物で、`hint`は「どう答えるか」、`lineJa`は「NPCが何を言っているか」を示す。ROLEPLAY_SCENES全175件に`npc`直後のフィールドとして追加（`roleplay-scene-edit`スキルの3視点＋独立エージェント検証を通過）。表示は`.rp-npc-bubble`内・`scene.line`の直下に常時表示（タップ開閉にしない — hintとは違い「セリフの意味」は毎回必要な情報でヒントではないため）。`renderRPChoose()`・`renderRPFreeResponse()`・MATCHDAY`renderMdScene()`の3箇所すべてに配線。10人視点で出た残り1案（英国サッカー俗語のタップ開閉グロッサリー）は未着手。もう1案（MATCHDAY自由回答の初回露出タイミング調整）は下記の`selfLevel`施策で対応済み。
  - **Tier1/BRONZE選択肢の日本語訳＋音声プレビュー（`keysJa`、2026-07-18）**: lineJaはNPCのセリフのみ対象で、3択の選択肢自体（`scene.keys`）には翻訳が一切無く、「状況とNPCのセリフは分かるが、なんと返すかの3択が読めない」という中学生ペルソナの離脱リスクが残っていた。`keys`と同じ並びの`keysJa`（3件配列）をROLEPLAY_SCENES全175件に追加（175件全部が"正解"の言い換え違いなので、3択は「間違い探し」ではなく「言い方を選ぶ」だけ、という前提から翻訳を出すこと自体に学習的デメリットは無いと判断）。`renderRPChoose()`の各選択肢ボタンに`lineJa`と同じ`.line-ja`常時表示で追加、あわせて既存の`scene.keyAudio`（元々はTier2+の模範読み上げ用）を選択前に聞けるプレビュー再生ボタン（`rpPlayChoice()`）として流用— 新規音声0件で「読めなくても聞いて選ぶ」を実現。選んだ後に自分の声で読み上げるステップ（`renderRPSpeak()`）は変更なし、英語production自体は今まで通り必須。
  - **自己申告レベル＋足場かけの初期値（`S.selfLevel`、2026-07-18）**: 「選手とのオンライン英会話で、今のアプリは英語初心者にはスタンダードが高すぎると感じた」というユーザーの実体験を起点に10人視点で検討。Duolingoのような詳細な適性判定テストは見送り（TOP BINSは175シーン固定・Day1〜25の単一トラック構造で、判定結果を投げ込む「簡単な方のコンテンツ」が存在しないため、テストを作っても着地点が無い）。代わりに**1問だけの自己申告**（`showFirstRun()`直後、`frShowLevel()`→`frSelectLevel()`）を追加し、新規コンテンツを作らず**既存の足場かけの初期値を変えるだけ**に留めた: ①`selfLevel==='beginner'`なら`renderRPFreeResponse()`のTier2+ヒントをデフォルト展開（`beginnerOpen`、通常はタップ開閉）、②MATCHDAYの本番プレイ（`openMatchday()`）をROLEPLAY Day `MD_BEGINNER_UNLOCK_DAY`(=3)未満では受け付けずROLEPLAYに誘導（無料お試しDEMOはentitledに関係なく従来通り即座に遊べる、ここは対象外）、③Day1初回だけ`startCards('pitch')`（単語カード）を挟んでからROLEPLAYを始めさせる（`S.rpVocabWarmupDone`で一度きり）。**自己申告の「盛り」を実績で静かに補正する**: `finishRoleplay()`でDay1〜3（`rpDay<3`）に`selfLevel==='intermediate'`かつその日の平均スコアが40点未満（`renderRPDayComplete()`の"KEEP GOING"帯と同じ閾値）なら`S.selfLevel`を`'beginner'`へ自動ダウングレード（`S.selfLevelAuto`で一度きり、本人に通知はしない）。Day1〜3はほぼ全員Tier1/BRONZEでヒント自体が存在しないため、判定はヒント使用率ではなくスコアのみを見ている。自己申告をスキップしたユーザー（`selfLevel===''`）は対象外（スキップという選択を尊重し、スキャフォールディングのデフォルトは変えない）。
- **MATCHDAY**: 1日1問・MATCH TICKET制（無料は`MD_TICKET_MAX`=1枚、entitledは無制限）。**失敗駆動復習**: AI採点<50で `S.mdReviewQueue` に登録（初回失敗→翌日、再失敗→3日後）、≥50で卒業。`shuffleMd()` は復習期限が来たシーンを最優先で出題。シーンのキーは `${cat}||${title}` 形式で統一。
  ...
  - **命令文・宣言文シーンの疑問文への入れ替え（2026-07-18）**: ナッジ/コンボ/語彙可視化を実装してもなお「"Yes"/"Got it"で完結してしまうシーンが多すぎる」というユーザー指摘を受け、根本原因である`line`自体の文型に対応。**MATCHDAYの出題プールは`S.rpCompletedSceneKeys`（ROLEPLAYで完了済みのシーン）からしか選ばれない**（`shuffleMd()`）ため、新規シーンを単純追加してもROLEPLAYのDay 1〜25ローテーションに組み込まれない限りMATCHDAYには一生出てこない。さらにDay構造は`175=7カテゴリ×25`が前提（`rpDayScenes()`はカテゴリごとの配列インデックスでDayを引く）のため、単純追加でカテゴリ件数が不揃いになるとDay構造が壊れる。**新規シーン追加ではなく、各カテゴリで最もYes/No的な命令文・宣言文シーンを選び、疑問文ベースの新規シーンと1:1で置き換える**方式を採用（175件・25/カテゴリを維持、Day構造・MATCHDAYプールとも無傷）。対象30件はユーザーがxlsxで英文を手直しした上で確定。差し替え後は`roleplay-scene-edit`スキルの3視点レビュー（配列カウント・タイトル重複なし・`git diff`で対象外フィールド無変化・音声整合性）を通過。新規音声30シーン×4フレーズ（`line`+`keys`3件）=120件は`scripts/phrases_to_record.txt`に追記して`elevenlabs_generate.js`で生成済み。
...
- **録音ボタンの`onclick`をJSで直接swapする実装は、停止後に必ず元に戻すこと**: `mdStartRec()`は録音開始時に`#md-mic-btn`の`onclick`を`mdStopRec`に、背景色を赤に書き換える設計だが、`mdShowNudge()`（MATCHDAYの「もう一言足せる？」ナッジ画面）と`mdNudgeRetry()`はこれを元（`onclick=mdStartRec`、背景リセット）に戻していなかったため、ナッジ表示後はマイクが赤いまま操作不能になる事故があった（2026-07-17発見・修正）。`mdStopRec()`の「録音機が既にinactive」フォールバック分岐でも同様に見落としていた。RP/ドリルは`toggleXxxRec()`という単一の状態判定エントリポイントで開始/停止を切り替える設計（`onclick`固定）のため同種の事故が起きない — 新しい録音UIを作る時はこちらのパターンを優先し、`onclick`を直接書き換える場合は停止・分岐が発生する**全箇所**で元に戻すことを確認する。
- **`100dvh`は外部ブラウザUI（アドレスバー等）が表示された状態を正しく差し引けない場合がある**: `html,body`/`#app`/`.screen`の高さに`100dvh`を使っていたところ、ブラウザのツールバーが表示された瞬間の初期描画で`dvh`が「ツールバーが隠れた場合」を想定した大きめの値になり、実際に見えている範囲より内部的な高さが大きく計算される事故があった（2026-07-17発見・修正、MATCHDAYのHINTボックスが画面外に押し出されスクロールもできなくなる不具合として発現）。`.screen`自身は`overflow-y:auto`だが、コンテンツの実高さがこの「大きすぎるdvh」に収まってしまうため、CSS上はoverflowが一切発生せずスクロールバーも生成されない（`scrollHeight===clientHeight`のまま）。`100svh`（=ツールバー表示時の最小保証サイズ）に変更し、常に「今実際に見えている範囲」に収まるよう修正。ツールバーが後から隠れた場合は下に余白ができるだけで、逆方向（見えない領域が発生する）事故は起きない。
