...
- **MAKE IT YOURS（Dayの締め）**: サブ理念「自分事に置き換えて覚える」（自己関連付け効果＋生成効果）の実装。Day完了時に今日のフレーズ1つの「形」を借りて自分の本当のことをテキストで下書き→声に出して読み上げ→AIが文法チェック（`/api/score` 経由）→修正版を `S.myPhrases` に既存形式 `{c,en,ja,ex,exJa,note}` で保存（単語カードの「マイフレーズ」デッキに自動還流）。聞き直しは録音Blob（コストゼロ）＋ブラウザTTSお手本。スキップ可・強制しない。
  - **「形を借りて」だけでは何をすればいいか伝わらない問題への対応（2026-07-12）**: 8人視点レビューで「英語力が低いユーザー層には自由生成のハードルが高すぎる」と判明。`MIY_EXAMPLES`（7カテゴリに1つずつ、日本語ガイド＋埋め込み例文のペア）を`_miyBase.cat`で引いて、ベースフレーズの下に「たとえばこう変えられる」という具体例を常時表示するようにした。175シーン個別ではなく7カテゴリ分だけ手動用意することで低コストに抑えている。**この例文のキー名を`en:`にしないこと**（`sample:`を使用）— `validate_audio.js`は`en:`という文字列を全ファイル横断でスキャンして音声必須と判定するため、音声を持たない静的な参考テキストに`en:`を使うと`npm run verify`が誤って「mp3が存在しない」と警告する。
  - **作文と発話を同時に要求しない2段階フロー（2026-07-12）**: 「いきなり口に出すのはハードルが高い」という指摘を受け、`renderMIY()`（Step1・テキストで下書き）→`renderMIYSpeak()`（Step2・書いた文をそのまま読み上げる）に分割。Step2の録音は`keepRec(blob)`で「自分の声を聞く」用に保存するだけで、**Whisper文字起こしはもう呼ばない**（`miyCheck()`にはStep1で書いたテキストをそのまま渡す）。作文済みの文を音声認識で誤って壊すリスクを構造的に排除しつつ、Whisper呼び出し1回分のAPIコストも削減できた。
  - **スキップしても書いた文は無駄にしない**: 8人視点レビューで「ステップを2つに割ると離脱ポイントも2箇所に増える、発話をスキップすると書いた文が消えるのは損失が大きい」と指摘され対応。`miySkip()`は`_miyDraft`（またはStep1でtextareaに入力中の未確定テキスト）があれば`saveDraftPhrase()`でAI文法チェックを経て`S.myPhrases`に保存する（`note`に「（下書き）」を付けて完走分と区別）。発話（Step2）まで到達しなくても、作文の努力そのものは失われない。AI呼び出しロジックは`miyGrammarCheck()`に共通化し、`miyCheck()`と`saveDraftPhrase()`の両方から使う。
